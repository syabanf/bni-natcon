-- The organiser's own stand joins the floor: "BNI Indonesia" (sheet rev. 31
-- Aug) has no booth code, so its scanner signs in with the email on the
-- sheet — operation.bniindonesia@gmail.com — and its stand label stays
-- empty. For databases that already ran the earlier booth migrations (0037
-- is keyed by filename and will not run again there), this replays the
-- regenerated, re-runnable booth SQL; on a fresh database every statement
-- here matches nothing new.

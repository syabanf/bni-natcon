...
func (r *AdminRepo) Overview(ctx context.Context) (*domain.AdminOverview, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		Scan(&o.TotalMembers, &o.TotalTenants, &o.TotalVisits, &o.VisitsToday,
...
func (r *AdminRepo) TenantRanking(ctx context.Context) ([]domain.TenantScanCount, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.Name, &t.Category, &t.Booth, &t.Initials, &t.OwnerUserID, &t.ScanCount); err != nil {
			return nil, err
		}
		out = append(out, t)
	}

...
func (r *TenantRepo) ListWithVisits(ctx context.Context, memberID int64) ([]domain.TenantWithVisit, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT t.id, t.name, t.category, t.booth, t.initials, t.owner_user_id,
		       (v.id IS NOT NULL) AS visited
		FROM tenants t
		LEFT JOIN visits v ON v.tenant_id = t.id AND v.member_id = $1
		ORDER BY t.booth`, memberID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.Name, &t.Category, &t.Booth, &t.Initials, &t.OwnerUserID, &t.Visited); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
...
func (r *TenantRepo) GetByOwnerUserID(ctx context.Context, ownerUserID int64) (*domain.Tenant, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, name, category, booth, initials, owner_user_id
		FROM tenants WHERE owner_user_id = $1`, ownerUserID).
		Scan(&t.ID, &t.Name, &t.Category, &t.Booth, &t.Initials, &t.OwnerUserID)
	if err != nil {
